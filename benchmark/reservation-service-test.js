...
// Write workload - POST create reservation
export function writeWorkload() {
    ...
    for (let i = 0; i < numSeats; i++) {
        seatIds.push(Math.floor(Math.random() * 1000) + 1);
    }

    const payload = JSON.stringify({
        showId: showId,
        seatIds: seatIds,
        totalAmount: numSeats * 10.0,
    });
    ...
    const res = http.post(`${BASE_URL}/api/v1/reservations?userId=${userId}`, payload, params);

    const result = check(res, {
        "create reservation - status is 201 or 200 or 400": (r) =>
